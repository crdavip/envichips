...
import { getDateRange, type DateRange } from "@/lib/services/informes";
...
import { ArrowDownCircle, ArrowLeft, ArrowUpCircle, DollarSign, Wallet } from "lucide-react";
...
async function CajaContent({
  ...
  const { desde: fechaDesde, hasta: fechaHasta } = getDateRange(
    rango as DateRange,
    customDesde,
    customHasta,
  );
  ...
  const [{ movimientos }, resumen] = await Promise.all([
    getMovimientos({ limit: 1000, fechaDesde, fechaHasta }),
