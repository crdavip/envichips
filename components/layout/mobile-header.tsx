...
export function MobileHeader({ userName, userEmail, userRole }: { userName: string; userEmail: string; userRole?: string }) {
  return (
    <header className="fixed top-0 left-0 right-0 z-30 flex h-14 items-center justify-between border-b bg-sidebar px-4 md:hidden">
      ...
      <div className="flex items-center gap-2">
        ...
        <UserMenu position="header" userName={userName} userEmail={userEmail} userRole={userRole} />
      </div>
    </header>
  );
}
